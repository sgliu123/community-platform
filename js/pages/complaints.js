...
function renderComplaints(){
  ...
  Promise.all([loadMyComplaints(), loadComplaintsFromWorker()]).then(([myList, allList])=>{
    ...
    if(getRoute().page==='complaints'){
      document.getElementById('main').innerHTML='<div class="page-content">'+renderComplaintsHTML(myList, allList)+'</div>';
    }
  }).catch(e=>{
    console.error(e);
    appData._complaints=[];
...
function renderSubmitComplaint(){
  ...
  h+='<button type="button" class="btn" style="padding:8px 16px;border:1px solid var(--border);border-radius:6px;background:#fff;cursor:pointer;font-size:13px;" onclick="document.getElementById(';
  h+="'cpImages'";
  h+=').click()">📎 选择图片</button>';
