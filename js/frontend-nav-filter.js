...
(function(){
  function getModuleSwitches() {
    var keys = ['adminData_config','config','app_config','community_config','adminData'];
    for (var i = 0; i < keys.length; i++) {
      try {
        ...
        if (raw) {
          ...
          if (parsed && parsed.config && parsed.config.settings && parsed.moduleSwitches === undefined) {
            if (parsed.config.moduleSwitches) return parsed.config.moduleSwitches;
          }
        }
      } catch(e) {}
    }
    try {
      var all = localStorage.getItem('adminData');
      if (all) {
        var p = JSON.parse(all);
