...
async function saveModuleSwitches() {
  ...
  try {
    ...
    // 触发 storage 事件，让同页面其他标签页感知
    localStorage.setItem('_moduleSwitch_trigger', String(Date.now()));
  ...
  try {
    ...
    showToast('配置已保存并同步到云端', 'success');
  } catch(e) {
    console.error('云端同步失败:', e);
    // Worker 挂了，但本地已保存
    showToast('云端同步失败（Worker 连接异常），配置已保存到本地。前台刷新页面即可生效。', 'warning');
  } finally {
    showLoading(false);
  }
  renderSidebar();
...
function resetModuleSwitches() {
  if (confirm('确定恢复默认配置吗？所有模块将恢复为开启状态。')) {
    ...
    try {
      localStorage.setItem('adminData', JSON.stringify(appData));
      localStorage.setItem('adminData_config', JSON.stringify({ config: { moduleSwitches: {} } }));
      localStorage.setItem('config', JSON.stringify({ moduleSwitches: {} }));
      localStorage.setItem('_moduleSwitch_trigger', String(Date.now()));
    } catch(e) {}
    navigateTo('dev-tools');
    showToast('已恢复默认配置，请点击"保存配置"同步到云端', 'info');
  }
}
...
function renderDashboard() {
  ...
  var documents = appData.documents || [];
  var activities = appData.activities || [];
  var polls = appData.polls || [];
  var residents = appData.residents || [];
  var workorders = appData.workorders || [];
  var complaints = appData.complaints || [];

  var pinnedCount = announcements.filter(function(a){ return a.isPinned; }).length;
  var activePolls = polls.filter(function(p){ return p.status === '进行中'; }).length;
  var activeResidents = residents.filter(function(r){ return r.status === 'active'; }).length;
  var pendingWorkorders = workorders.filter(function(w){ return w.status === '待处理'; }).length;
  var pendingComplaints = complaints.filter(function(c){ return c.status === '待处理'; }).length;

  var html = '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:16px;margin-bottom:24px;">';
  html += '<div style="background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);color:#fff;padding:20px;border-radius:12px;">' +
    '<div style="font-size:28px;font-weight:700;">' + announcements.length + '</div>' +
    '<div style="font-size:13px;opacity:0.9;">📢 公告总数</div>' +
    '<div style="font-size:12px;opacity:0.7;margin-top:4px;">置顶 ' + pinnedCount + ' 条</div></div>';
  html += '<div style="background:linear-gradient(135deg,#f093fb 0%,#f5576c 100%);color:#fff;padding:20px;border-radius:12px;">' +
    '<div style="font-size:28px;font-weight:700;">' + documents.length + '</div>' +
    '<div style="font-size:13px;opacity:0.9;">📄 文件总数</div></div>';
  html += '<div style="background:linear-gradient(135deg,#4facfe 0%,#00f2fe 100%);color:#fff;padding:20px;border-radius:12px;">' +
    '<div style="font-size:28px;font-weight:700;">' + activities.length + '</div>' +
    '<div style="font-size:13px;opacity:0.9;">🎉 动态总数</div></div>';
  html += '<div style="background:linear-gradient(135deg,#43e97b 0%,#38f9d7 100%);color:#fff;padding:20px;border-radius:12px;">' +
    '<div style="font-size:28px;font-weight:700;">' + polls.length + '</div>' +
    '<div style="font-size:13px;opacity:0.9;">🗳️ 投票总数</div>' +
    '<div style="font-size:12px;opacity:0.7;margin-top:4px;">进行中 ' + activePolls + ' 个</div></div>';
  html += '<div style="background:linear-gradient(135deg,#fa709a 0%,#fee140 100%);color:#fff;padding:20px;border-radius:12px;">' +
    '<div style="font-size:28px;font-weight:700;">' + residents.length + '</div>' +
    '<div style="font-size:13px;opacity:0.9;">👥 业主总数</div>' +
    '<div style="font-size:12px;opacity:0.7;margin-top:4px;">正常 ' + activeResidents + ' 户</div></div>';
  html += '<div style="background:linear-gradient(135deg,#a8edea 0%,#fed6e3 100%);color:#333;padding:20px;border-radius:12px;">' +
    '<div style="font-size:28px;font-weight:700;">' + pendingWorkorders + '</div>' +
    '<div style="font-size:13px;opacity:0.9;">🔧 待处理工单</div></div>';
  html += '<div style="background:linear-gradient(135deg,#d299c2 0%,#fef9d7 100%);color:#333;padding:20px;border-radius:12px;">' +
    '<div style="font-size:28px;font-weight:700;">' + pendingComplaints + '</div>' +
    '<div style="font-size:13px;opacity:0.9;">📝 待处理投诉</div></div>';
  html += '</div>';

  // 快捷入口
  html += '<div class="card"><div class="card-header"><h3>⚡ 快捷入口</h3></div>';
  html += '<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:12px;">';
  var quickLinks = [
    {id:'announcements',label:'发布公告',icon:'📢'},
    {id:'documents',label:'上传文件',icon:'📄'},
    {id:'activities',label:'发布动态',icon:'🎉'},
    {id:'polls',label:'发起投票',icon:'🗳️'},
    {id:'residents',label:'业主管理',icon:'👥'},
    {id:'workorders',label:'工单处理',icon:'🔧'},
    {id:'complaints',label:'投诉处理',icon:'📝'},
    {id:'settings',label:'系统设置',icon:'⚙️'}
  ];
  quickLinks.forEach(function(link){
    html += '<button class="btn" onclick="navigateTo('' + link.id + '')" style="text-align:center;padding:16px;font-size:14px;">' +
      '<div style="font-size:24px;margin-bottom:4px;">' + link.icon + '</div>' + link.label + '</button>';
  });
  html += '</div></div>';

  // 最新公告
  if (announcements.length > 0) {
    html += '<div class="card" style="margin-top:16px;"><div class="card-header"><h3>📢 最新公告</h3></div>';
    ...
    recent.forEach(function(a){
      html += '<div style="display:flex;justify-content:space-between;align-items:center;padding:10px;border-bottom:1px solid var(--border);">' +
        '<div><span style="font-size:12px;color:var(--text-secondary);">[' + (a.category||'公告') + ']</span> ' + escapeHtml(a.title) + '</div>' +
        '<span style="font-size:12px;color:var(--text-secondary);white-space:nowrap;">' + (a.publishDate||'') + '</span></div>';
    });
    html += '</div>';
  }
